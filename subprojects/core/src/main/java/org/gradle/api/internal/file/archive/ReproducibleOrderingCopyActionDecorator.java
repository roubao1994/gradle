    ...
    public WorkResult execute(final CopyActionProcessingStream stream) {
        return delegate.execute(new CopyActionProcessingStream() {
            public void process(final CopyActionProcessingStreamAction action) {
                final SortedMap<RelativePath, FileCopyDetailsInternal> files = Maps.newTreeMap();
                stream.process(new CopyActionProcessingStreamAction() {
                    public void processFile(FileCopyDetailsInternal details) {
                        files.put(details.getRelativePath(), details);
                    }
                });
                for (Map.Entry<RelativePath, FileCopyDetailsInternal> e : files.entrySet()) {
                    action.processFile(e.getValue());
                }
            }
        });
    }
